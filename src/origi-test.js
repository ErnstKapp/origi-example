...
async function getImages(user, image_contents){
        const filehub = new Filehub(Config.fileHub.url, Config.fileHub.brid)
        images = []
        await registerUserOnFileHubIfNeeded(user)
        for (image_content of image_contents){
            let image_buffer = Buffer.from(image_content, 'hex')
            let image = await filehub.getFile(image_buffer)
            images.push(image)
        }
        return images
}
...
async function test(){
        ...
        await blockchain.call(op('bear.give_bear_tshirt', [user1.authDescriptor.id, user1.authDescriptor.id]), user1)    
        let res = []

        if (account != null){
          res = await blockchain.query('bear.get_last_n_minted_originals', {account_id : account})
          console.log(res);
        }
        else{
          return 0;
        ...
        for (elem in res){
          if(res[elem]['chromia.IOriginal.name'] == 'RedShirt'){
            let image_ID = res[elem]['chromia.ITradable.image']
            console.log(image_ID)
            image_list.push(image_ID[0])
          }
        }
        
        let images = await getImages(user1, image_list)
